...
class AddEntryButtonRow(QVBoxLayout):
    ...
    def __init__(self):
        ...
        self.__closeButton.clicked.connect(signalManager.on_new_entry_window_close.emit)
        self.__clearButton = IconButton("clear", 40)
        self.__clearButton.clicked.connect(signalManager.on_set_default_values.emit)
        self.__saveButton = IconButton("save", 40)
        self.__saveButton.clicked.connect(signalManager.on_save_entry_click.emit)
